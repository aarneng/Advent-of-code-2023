// input for each day is unused if the day has not been solved yet
...
fn main() {
    ...
    for day in days {
        let binding = read_to_string(get_day_filename(day))
            .expect(&format!("Failed to read the file for day {}", day));
        let split_input: std::str::Lines<'_> = binding.lines();

        let func = get_day_solver(day);

        let time = Instant::now();
        let (p1, p2) = func(split_input);
        let elapsed_ms = time.elapsed().as_secs_f64() * 1000.0;
...
fn get_day_solver(day: u8) -> fn(std::str::Lines<'_>) -> SolutionPair {
    match day {
         1 => day01::solve,
         2 => day02::solve,
         3 => day03::solve,
...
fn get_day_filename(day: u8) -> String {
